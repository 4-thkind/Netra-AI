...
files["backend/app/integrations/__init__.py"] = ""

files["backend/app/integrations/sarvam_adapter.py"] = '''import httpx
...
class SarvamAIAdapter:
    ...
    Multilingual Indic text & TTS generation (Hindi / Indic dialects).
    Minimizes context exposure and provides realistic audio and translations.
    """
    def __init__(self):
    ...
    async def translate_or_localize(self, text_en: str, target_lang: str = "hi") -> str:
        # If demo mode, return polished Hindi translations for Kirana merchants
        if settings.DEMO_MODE or self.api_key == "mock-sarvam-key":
            translations = {
                "Beverage demand is rising": "आपके क्षेत्र में ठंडे पेय पदार्थों (Beverages) की मांग बढ़ रही है।",
                "Snack transaction value is above category benchmark": "आपकी नमकीन और स्नैक्स की औसत बिक्री स्थानीय बाजार से अधिक है। एक कॉम्बो पैक बनाकर बेचें।",
                "Review beverage inventory": "दुपहर की मांग के लिए कोल्ड ड्रिंक्स का स्टॉक चेक करें।"
            }
            for k, v in translations.items():
                if k.lower() in text_en.lower():
                    return v
            return f"{text_en} (हिंदी अनुवाद उपलब्ध)"
        ...
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.post(
                    ...
                    json={"input": text_en, "source_language_code": "en-IN", "target_language_code": f"{target_lang}-IN"}
                )
                if resp.status_code == 200:
                    return resp.json().get("translated_text", text_en)
            except Exception:
                pass
        return text_en

    async def generate_soundbox_speech(self, text: str, language: str = "hi") -> Dict[str, Any]:
        """
        Generates simulated or real Sarvam TTS audio waveform for Paytm Soundbox playback.
        """
        return {
            "status": "ready",
            "voice": "hindi_male_soundbox",
            "audio_url": "/api/v1/merchant/audio/sample_signal.mp3",
            "transcript": text
        }
...
files["backend/app/integrations/cognee_adapter.py"] = '''from typing import Dict, Any, List
...
files["backend/app/integrations/n8n_adapter.py"] = '''from typing import Dict, Any
...
files["backend/app/integrations/paytm_adapter.py"] = '''from typing import Dict, Any
import random
import uuid
...
files["backend/app/data/__init__.py"] = ""

files["backend/app/data/synthetic_generator.py"] = '''import asyncio
import uuid
import random
        ...
        for c in CLUSTERS:
            for i in range(c["count"]):
                ...
                m = Merchant(
                    ...
                    city="Delhi",
                    cluster_id=c["id"],
                    hashed_password=hash_password("kirana123")
                )
                db.add(m)
